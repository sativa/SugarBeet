...
!     they are used/defined in COMMON blocks! (see PADDY subroutines)
      ...
      REAL       ULLS, LLLS, ULDL, LLDL, LLLE, ULLE, LLRT, ULRT
		  ...
		  REAL FSWTD, OSMATA1    !CROP OSMATIC POTENTIAL ADJUSTMENT, SUPPOSED THE FSWTD = 1.5 MP IS THE NORMAL DROUGHT TOLERNACE
		  							 !IF FSWTD = 3.0 MP, THEN WATER UPTAKE ABILITY WILL BE IMPROVED SIGNIFICANTLY WHILE SOIL WATER IS
		  							 !LOWER THEN FIELD CAPACITY. TAOLI, 26 FEB 2010
		  REAL, ALLOCATABLE::RUPAC1(:), RUPAC2(:), RUPAC3(:), RUPAC4(:) !ROOT UPTAKE ACTIVITY IN EACH LAYER, TAOLI, 26 FEB 2010
		  REAL Fintegration
      REAL LIMIT 
      REAL TINY
...
!----------update public variables for soil water uptake
            ...
            END DO
           PCEW   = (TRW/TRC)
       ELSEIF(SWIRTR.EQ.3) THEN   !use selfsdjutment function, root and water interaction for quatifying available water uptake
