...
!  Modified by Tao Li, 17 Aug, 2010                                    *
!                                                                      *
...
! SFLOWER R4  The starting hour of flowering in a day				I  *  
! IFLOWER R4  The maximum flowering period in a day                 I  *
! TFLOWER R4  The critical temperature for spiklete sterility       I  * 
...
!  FILE usage : none                                                   *
!----------------------------------------------------------------------*
      SUBROUTINE SUBGRN3(GCR,CROPSTA,LRSTRS, DVS,SF2,SF1,SPGF,TAV,TMAX, &
...
!-----Formal parameters
      ...
      REAL    SPFERT, LRSTRS, SFlower,IFlower,TFlower, TMIN, TEMPERATURE, J
      INTEGER CROPSTA
      LOGICAL GRAINS
!-----Local parameters
...
!-----Calculate GNGR reduction factors
      ...
      IF ((DVS.GE.0.96).AND.(DVS.LE.1.2)) THEN
		 TMIN = TMAX - (TMAX-TAV)*2.0
		 DO J = SFLOWER, SFLOWER+IFLOWER,0.5
			TEMPERATURE = 0.0
			TEMPERATURE = (TMIN+TMAX)*0.5+(TMAX-TMIN)*COS(0.2618*(J-14.0))*0.5
			TFERT  = TFERT +((TEMPERATURE+TINCR)-TFLOWER)*0.5
		 END DO
...
!-----Apply GNGR reduction factors when DVS is 1.2
      IF ((DVS.GE.1.2).AND.(.NOT.GRAINS)) THEN
         ...
         SF1    = MIN(1.,MAX(0.,SF1))
         TFERT  = TFERT/(NTFERT)
         !SF2    = 1./(1.+EXP(0.853*(TFERT-TFLOWER)))		!THE DEFAULT TFLOWER=36.6
		 SF2 = 1.0/(1.0+0.904677*EXP(0.0794979*TFERT))
         SF2    = MIN(1.,MAX(0.,SF2))
         SPFERT = MIN(SF1,SF2)
         GNGR   = NSP*SPFERT
      ELSE
