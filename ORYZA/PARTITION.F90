SUBROUTINE PARTITION_K(EXTICK,LAI,MAXLAI,FCLIMAT,FWATER,FNITRO,OSMATA,ISTRESS,FSH,FRT,FLV,FST,FSO)
!-------------------------------------------------------------------------------
...
!OSMATA		REAL		the maximum osmatic potential of plant													-
...
!--------------------------------------------------------------------------------
REAL EXTICK,LAI,MAXLAI,FCLIMAT,FWATER,FNITRO,OSMATA,FSH,FRT,FLV,FST,FSO, FLIGHT
INTEGER ISTRESS
REAL TMPV1, tmpS, tmpL
	 ...
	 IF(((FLV+FST+FSO).GT.0.99).AND.((FLV+FST+FSO).LT.1.01)) THEN
	 ELSE
	 ...
	 ENDIF
	 	...
	 	tmps=1.0-(1.0-min(fnitro, fwater))**(OSMATA*1.5)   !4.0
	 	tmpl=1.0-(1.0-flight)**4.0
   	FSH = FSH /(1.0+FCLIMAT*(1.0-tmps))
   	FRT = 1.0 - FSH
   IF(((FSH+FRT).LT.1.01).AND.((FSH+FRT).GT.0.99)) THEN
   ELSE
