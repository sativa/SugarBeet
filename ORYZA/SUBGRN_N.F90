...
!  FILE usage : none                                                   *
!----------------------------------------------------------------------*
      SUBROUTINE SUBGRN(GCR,CROPSTA,LRSTRS, DVS,SF2,SF1,SPGF,TAV,TMAX, &
...
!-----Formal parameters
      ...
      REAL    SPFERT, LRSTRS, maxFlower, TMIN
      INTEGER CROPSTA
      LOGICAL GRAINS
!-----Local parameters
...
!-----Calculate GNGR reduction factors
      ...
      IF ((DVS.GE.0.96).AND.(DVS.LE.1.2)) THEN
		 ...
		 maxFLOWER= SFLOWER + 0.5*IFLOWER
		 TMAX = (TMIN+TMAX)*0.5+(TMAX-TMIN)*COS(0.2618*(maxFLOWER-14.0))*0.5
