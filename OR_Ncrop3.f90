...
!+				here, the effects of avaliable water content on N uptake should be added in
			...
			DO I=1,SL
				...
				IF(REFFECD.GT.SDEP2) THEN
					tmpv1 =MAX(0.0,SNH4X(I)-SMINNH4*BD(I)*LAYT(I)*10.0)    !SMINNH4 AND SMINNO3 ARE MG N /KG SOIL, SNH4X IN KG/HG, LAYT IN M
					...
					IF((TMPV3.GT.0.0).AND.(TMPV4.GT.0.0)) THEN
						SPNH4(I) = MAX(0.0,tmpv1) /tmpv3* pv%PTrwl(i)	!so far, it only calculate the mass flow uptake
						SPNO3(I)=MAX(0.0,tmpv2) /tmpv3* pv%PTrwl(i)		!It is also needed to calculate the difussion flow
					ELSE
						SPNH4(I) = 0.0
						SPNO3(I)=0.0
					ENDIF
				...
				ELSEIF((REFFECD.GT.SDEP1).AND.(REFFECD.LE.SDEP2)) THEN
					tmpv1 =MAX(0.0,(SNH4X(I)-SMINNH4*BD(I)*LAYT(I)*10.0)*(REFFECD-SDEP1)/LAYT(I))
					tmpv2 =MAX(0.0,(SNO3X(I)-SMINNO3*BD(I)*LAYT(I)*10.0)*(REFFECD-SDEP1)/LAYT(I))
					tmpv3 = tswc(i)*(REFFECD-SDEP1)*1000.0
					IF((TMPV3.GT.0.0).AND.(TMPV4.GT.0.0)) THEN
						SPNH4(I) = MAX(0.0, tmpv1) /tmpv3*pv%PTrwl(i)
						SPNO3(I)=MAX(0.0,tmpv2) /tmpv3*pv%PTrwl(i)
					ELSE
						SPNH4(I) = 0.0
						SPNO3(I)=0.0
					ENDIF	
...
!        Output writing
         ...
         CALL ChartOutputRealScalar('NSLLV', NSLLV)
           CALL OUTDAT (2, 0, 'RNSTRS', RNSTRS)
         CALL ChartOutputRealScalar('RNSTRS', RNSTRS)
         ...
         CALL ChartOutputRealScalar('SNH4', SNH4X(1))
           CALL OUTDAT (2, 0, 'SNO3', SNO3X(1))
         CALL ChartOutputRealScalar('SNO3',  SNO3X(1))
         END IF
