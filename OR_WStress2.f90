...
!     they are used/defined in COMMON blocks! (see PADDY subroutines)
      ...
      REAL       ULLS, LLLS, ULDL, LLDL, LLLE, ULLE, LLRT, ULRT
		  REAL TRUPAC, TROOTF, TWATERF  !TOTAL ROOT UPTAKE ACTIVITITY, fraction of root and water, TAOLI, 26 FEB 2010
		  REAL OSMATA, OSMATA1    !CROP OSMATIC POTENTIAL ADJUSTMENT, SUPPOSED THE OSMATA = 1.5 MP IS THE NORMAL DROUGHT TOLERNACE
		  							 !IF OSMATA = 3.0 MP, THEN WATER UPTAKE ABILITY WILL BE IMPROVED SIGNIFICANTLY WHILE SOIL WATER IS
		  							 !LOWER THEN FIELD CAPACITY. TAOLI, 26 FEB 2010
		  REAL, ALLOCATABLE::RUPAC1(:), RUPAC2(:), RUPAC3(:) !ROOT UPTAKE ACTIVITY IN EACH LAYER, TAOLI, 26 FEB 2010
		  REAL Fintegration
      REAL LIMIT 
      REAL TINY
...
!        Read initial states (Note: values in kPa)
         ...
         END IF
		 IF(SWIRTR.EQ.'FUNCTION') THEN		!ADDED BY TAOLI, 31 MAY, 2010
         ...
         END DO
         TRW = 0.
	Open(unit = 500, FILE = "D:\ModelPrograms\CVF_ORYZA3\test.txt")
!====================================================================!
...
!-------------Relative death rate factor
          ...
          !-----Set stress factors as average over all layers: DEFAULT
           ...
           LDSTRS = LDAV
           LESTRS = LEAV
        !USE different function for trnaspiration. TAOLI 26 FEB 2010   
       IF(.NOT. PV%PROOT_NUTRIENT) THEN
...
!----------update public variables for soil water uptake
            ...
            END DO
           PCEW   = (TRW/TRC)
       ELSE   !use selfsdjutment function, root and water interaction for quatifying available water uptake
       				!Supposed the root could absorbe all avaliable water if it need, no maximum limitation. TAOLI 26 FEB 2010
        ALLOCATE(RUPAC1(NL), RUPAC2(NL), RUPAC3(NL))
        ...
        DO I=1, NL			 	
         ...
         DO I = 1, NL
         	RUPAC2(I) = ROOTC(I)/TROOTF	!Before 27 Aug 2010
			!RUPAC3(I) = RUPAC3(I)/ TWATERF	!Before 27 Aug 2010
			RUPAC1(I) = RUPAC3(I)/ TWATERF	!AFTER DECEMBER 2011
			!TRUPAC = TRUPAC + RUPAC3(I)*RUPAC2(I)	!Before 27 Aug 2010
			TRUPAC = TRUPAC + RUPAC1(I)*RUPAC2(I)  !AFTER DECEMBER 2011
         ENDDO
         TWATERF = TRUPAC; TRUPAC = 0.0
         !CALCULATE TOTAL AVALIABLE WATER CAN BE UPTAKEN
         DO I = 1, NL
			 !RUPAC2(I)=RUPAC2(I)*RUPAC1(I)      !Before 27 Aug 2010
         	 !RUPAC3(I)= RUPAC3(I) * RUPAC2(I)		!Before 27 Aug 2010
			 !TRUPAC = TRUPAC + RUPAC3(I)			!Before 27 Aug 2010
			 XXXX=RUPAC3(I)	
			 RUPAC3(I)=RUPAC3(I) * RUPAC2(I)*RUPAC1(I)/TWATERF !AFTER DECEMBER 2011 
				RUPAC1(I)=XXXX;RUPAC2(I)=WCLQT(I)-PV%PWCWP(I)
			TRUPAC = TRUPAC + RUPAC3(I) !AFTER DECEMBER 2011		
         ENDDO
         ...
         IF(TRC.GT.0.0) THEN
         	IF(TRC.GE.TRUPAC) THEN
         		TRW = TRUPAC
         		DO I = 1, NL
         			TRWL(I) = RUPAC3(I)/DELT
         		ENDDO
            ELSE
         		TRW =TRC; TROOTF = TRC/TRUPAC
         		DO I = 1, NL
         			TRWL(I) = RUPAC3(I) * TROOTF/DELT
         		ENDDO
            ENDIF 
         ENDIF
!--------UPDATE the soil water uptake
		 ...
		 write(500, 5000) TRC,TRUPAC,RUPAC3,trwl,zrt
		 5000 format(40(f12.8)) 
         !Calculating the drought stress factors on transpiration, temporal function, will use GECROS function
!         PCEW   = (NOTNUL(TRW/TRC))**(3.0/OSMATA1)			! Before 27 Aug, 2010
		 PCEW   = 1.0-(1.0- NOTNUL(TRW/TRC))**OSMATA			! After 27 Aug, 2010
!		 PCEW   = (NOTNUL(TRW/TRC))					! After 27 Aug, 2010
		 DEALLOCATE(RUPAC1, RUPAC2, RUPAC3)   
       endif
!-------If crop is not in the main field, set all stres factors at 1.
     ELSE
	   ...
	   END IF
        ...
        CPEW = LESTRS
!============================================================*
!Temporary for check TRWL, TAOLI, 21 MARCH 2010
!			CALL TOUTDATA(pv%CRUN+6, 2, 'DOY', DOY)
!			CALL TOUTDATA(pv%CRUN+6, 2, 'DVS' , DVS)
!			CALL TOUTDATA(pv%CRUN+6, 2, 'PCEW', PCEW)
!			CALL TOUTDATA(pv%CRUN+6, 2, 'LRSTRS', LRSTRS)
!			CALL TOUTDATA(pv%CRUN+6, 2, 'CPEW', CPEW)
!			CALL TOUTDATA(pv%CRUN+6, 2, 'LDSTRS' , LDSTRS)
!			CALL TOUTDATA(pv%CRUN+6, 2, 'LESTRS', LESTRS)
!			CALL TOUTDATA(pv%CRUN+6, 2, 'PCEW', PCEW)
!			CALL TOUTDATA(pv%CRUN+6, 2, 'LRSTRS', LRSTRS)
!			CALL TOUTDATA(pv%CRUN+6, 2, 'TRWL1', TRWL(1))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'TRWL2', TRWL(2))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'TRWL3', TRWL(3))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'TRWL4', TRWL(4))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'TRWL5', TRWL(5))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'TRWL6', TRWL(6))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'TRWL7', TRWL(7))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'MSKPA1', WCLQT(1))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'MSKPA2', WCLQT(2))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'MSKPA3', WCLQT(3))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'MSKPA4', WCLQT(4))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'MSKPA5', WCLQT(5))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'MSKPA6', WCLQT(6))
!			CALL TOUTDATA(pv%CRUN+6, 2, 'MSKPA7', WCLQT(7))
!------Output writing 
      IF (OUTPUT) THEN
...
!use numerical_libraries  !CHP 1/25/2011
!USE 
USE WSTRESS_MODULE
REAL theWCWP, theWCFC, theOSMATA, theUP
INTEGER NOUT
REAL A, ABS, B, ERRABS, ERREST, ERROR, ERRREL, EXACT, F, RESULT1
INTRINSIC ABS
EXTERNAL F 
...
ERRABS = 0.0
ERRREL = 0.001
CALL QDAGS (F, A, B, ERRABS, ERRREL, RESULT1, ERREST)
! Print results
EXACT = 0.0
ERROR = ABS(RESULT1-EXACT)
Fintegration = RESULT1
END FUNCTION
!
REAL FUNCTION F(X)
use WSTRESS_MODULE
