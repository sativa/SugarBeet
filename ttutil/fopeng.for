...
*     ---------------
      ...
      IF (LSTAT.EQ.'OLD'.AND.LPRIV.NE.' ') THEN
         CALL TT_WARNING ('FOPENG',     !CHP changed to TT_WARNING
...
c     &   ACCESS='SEQUENTIAL',FORM=FFF(1:ILF),IOSTAT=IOS)
c      IF (.NOT.SEQ) OPEN (IUNIT,FILE=LFNAME(1:ILFIL),STATUS=LSTAT,
c     &   ACCESS='DIRECT',FORM=FFF(1:ILF),RECL=IWL,IOSTAT=IOS)
...
*     =====================================
      IF (.NOT.SEQ) THEN
         IF (FFF.EQ.'FORMATTED') THEN
            IWL = IRECL
         ELSE IF (FFF.EQ.'UNFORMATTED') THEN
            IWL = 1 + (IRECL-1)/4
         END IF
      END IF

*     normal open
      IF (SEQ .AND. FFF.EQ.'FORMATTED'.AND.LSTAT.EQ.'NEW') THEN
*        sequential formatted file with max record length 512
         OPEN (IUNIT,FILE=LFNAME(1:ILFIL),STATUS=LSTAT,
     &    ACCESS='SEQUENTIAL',FORM='FORMATTED',
     &    BLOCKSIZE=4096,RECL=4096,IOSTAT=IOS,
     &    CARRIAGECONTROL='LIST')
      ELSE IF (SEQ .AND. FFF.EQ.'FORMATTED'.AND.LSTAT.EQ.'OLD') THEN
*        sequential formatted file with max record length 512
         OPEN (IUNIT,FILE=LFNAME(1:ILFIL),STATUS=LSTAT,
     &    ACCESS='SEQUENTIAL',FORM='FORMATTED',
     &    BLOCKSIZE=4096,RECL=4096,IOSTAT=IOS)
      ELSE IF (SEQ .AND. FFF.EQ.'UNFORMATTED') THEN
*        sequential unformatted file
         OPEN (IUNIT,FILE=LFNAME(1:ILFIL),STATUS=LSTAT,
     &    ACCESS='SEQUENTIAL',FORM='UNFORMATTED',
     &    BLOCKSIZE=4096,IOSTAT=IOS)
      ELSE IF (.NOT.SEQ) THEN
*        direct access file
         OPEN (IUNIT,FILE=LFNAME(1:ILFIL),STATUS=LSTAT,
     &    ACCESS='DIRECT',FORM=FFF(1:ILF),RECL=IWL,
     &    BLOCKSIZE=4096,IOSTAT=IOS)
      END IF
